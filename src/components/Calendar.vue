...
export default {
    ...
    return {
      ...
      events: [
        {
          ...
          start: "2019-11-08T19:00",
          duration: "01:00",
          backgroundColor: "#F9AA0D",
          firstName: "Vahagn",
          lastName: "Melkonyan",
          message: "verevi dzaxic 5-rd atami plomb piti drvi",
          phone: "055599565"
        },
        {
          id: 1,
      ...
      config: {
        ...
        eventMouseover: (event, jsEvent) => {
          ...
          this.cloudOptions.eventStartTime =
            "Starts at` " +
            ...
            (date.getMinutes() >= 10
              ? date.getMinutes()
              : "0" + date.getMinutes());
          this.cloudOptions.eventOwnerName =
            event.firstName + " " + event.lastName;
          this.cloudOptions.eventCloudMessage = event.message;
          topCoord =
            jsEvent.clientY + 200 > window.innerHeight
              ? jsEvent.clientY - 200
              : jsEvent.clientY;
          leftCoord =
            jsEvent.clientX + 350 > window.innerWidth
              ? jsEvent.clientX - 350
              : jsEvent.clientX;
        ...
        eventMouseout: () => {
          setTimeout(() => {
            if (!this.isCloudOver) this.flagForCloudVisibility = false;
          }, 2);
        }
      }
    };
  ...
  watch: {
    ...
    mySize() {
      document.getElementsByClassName("calendar_cont")[0].style.width =
        this.mySize + "%";
    },
    config() {
  ...
  mounted() {
    ...
    setInterval(() => {
      ...
      this.events.forEach(el => {
        if (
          new Date(el.start).getTime() - date <= 60000 * this.notifyMeBefore &&
          new Date(el.start).getTime() - date >= 0 &&
          el.groupId == "accepted"
        ) {
          el.backgroundColor = "yellow";
          let date = new Date(el.start);
          let string = `a${el.id} ${
            this.messageForUpcomingEvent
          } that starts at \` ${(date.getHours() >= 10
            ? date.getHours()
            : "0" + date.getHours()) +
            ":" +
            (date.getMinutes() >= 10
    ...
    this.events.forEach(el => {
      if (el.groupId == "waiting") {
        let date = new Date(el.start);
        let string = `b${el.id} ${el.firstName +
          " " +
          el.lastName} wants to register a meeting at \` ${date.getDate()} ${
  ...
  methods: {
    ...
    hideCloud() {
      if (this.flagForCloudVisibility) this.flagForCloudVisibility = false;
